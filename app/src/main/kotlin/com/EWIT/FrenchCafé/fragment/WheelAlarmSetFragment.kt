...
import com.EWIT.FrenchCafé.interfaces.AlarmSetInterface
...
class WheelAlarmSetFragment : Fragment(), AlarmSetInterface {
    ...
    private fun initInstance() {
        ...
        alarmDao = Model.AlarmDao(currentDate, currentTime, repeatDayList as ArrayList<Int>)
    ...
    var btnSetAlarmListener = {
        alarmDao = Model.AlarmDao(currentDate, currentTime, repeatDayList as ArrayList<Int>)
        setAlarm(alarmDao)
    }

    var timeChangedListener = { timePicker: TimePicker, hour: Int, min: Int ->
        currentTime = Model.TimePicked(hour, min)
        tvPickTime.text = "Pick time ${currentTime.getTimeFormat()}"
    }

    var btnSelectDateListener = {
